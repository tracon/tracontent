...
class Setup(object):
    ...
    def setup(self):
        print('NOTE: Setting up Tracon (2018) site at {domain}'.format(domain=self.domain))
        self.setup_site()
        self.setup_content()
        self.setup_ads()
        self.setup_blog()
    ...
    def setup_content(self):
        ...
        self.site_settings, unused = SiteSettings.objects.get_or_create(
            site=self.site,
            defaults=dict(
                base_template='tracon2018_base.jade',
                page_template='tracon2018_page.jade',
                blog_index_template='tracon2018_blog_index.jade',
                blog_post_template='tracon2018_blog_post.jade',
            )
        )
        ...
        for page_slug, page_title, child_pages in [
            ('front-page', 'Tracon Tampere-talossa 7.–9.9.2018', []),
        ...
        if not front_page.override_page_template:
            front_page.override_page_template = 'tracon2018_front_page.jade'
        ...
        programme_miitit_page, created = Page.objects.get_or_create(
            ...
            defaults=dict(
                ...
                public_from=t,
                visible_from=t,
                page_controller_code='site_specific.tracon2018.views:miitit_page_controller',
                override_page_template='tracon2018_programme_page.jade',
                order=0,
            )
        )

        for path, template, controller in [
            ('yhteys/conitea', 'tracon2018_organizers_page.jade', 'site_specific.tracon2018.views:organizers_page_controller'),
            ('tapahtuma/taidekuja', 'tracon2018_artists_alley_page.jade', 'site_specific.tracommon.views:artists_alley_page_controller')
        ]:
    ...
    def setup_ads(self):
        for banner_title, banner_url, banner_path in [
            ('Säätöyhteisö B2 ry', 'http://b2.fi', 'site_specific/tracon2018/static/tracon2018/img/b2-2016-200x-tr.png'),
