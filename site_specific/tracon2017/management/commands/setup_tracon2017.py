...
class Setup(object):
    ...
    def setup_content(self):
        ...
        self.site_settings, unused = SiteSettings.objects.get_or_create(
            site=self.site,
            defaults=dict(
                base_template='tracon2017_base.jade',
                page_template='tracon2017_page.jade',
                blog_index_template='tracon2017_blog_index.jade',
                blog_post_template='tracon2017_blog_post.jade',
            )
        )

        # v3
        if self.site_settings.page_template == 'tracon11_page.jade':
            self.site_settings.page_template = 'tracon2017_page.jade'
        if self.site_settings.blog_index_template == 'tracon11_blog_index.jade':
            self.site_settings.blog_index_template = 'tracon2017_blog_index.jade'
        if self.site_settings.blog_post_template == 'tracon11_blog_post.jade':
            self.site_settings.blog_post_template = 'tracon2017_blog_post.jade'

        self.site_settings.save()

        ordering = 0
        for page_slug, page_title, child_pages in [
            ('front-page', 'Tracon Tampere-talossa 8.–10.9.2017', []),
        ...
        if not front_page.override_page_template:
            front_page.override_page_template = 'tracon2017_front_page.jade'
        ...
        programme_miitit_page, created = Page.objects.get_or_create(
            ...
            defaults=dict(
                ...
                public_from=t,
                visible_from=t,
                page_controller_code='site_specific.tracon2017.views:miitit_page_controller',
                override_page_template='tracon2017_programme_page.jade',
                order=0,
            )
        )
